...
import {
  ...
  ListItemSecondaryAction, IconButton, Paper, Stack, Snackbar, 
...
import { AddOutlined, LockOutlined, SyncLockOutlined } from '@mui/icons-material';
...
export default function FileExplorer() {
  ...
  const [selectedFolderForPassword, setSelectedFolderForPassword] = useState('');
  ...
  const confirmDelete = (type, name, action) => {
    if(type === 'folder'){
      const confirm = window.confirm(`Bạn có chắc muốn xóa Folder "${name}" không`);
      if(!confirm) return;
    }
  ...
  const handleDeleteFile = (filename) => {
    confirmDelete('file', filename, async (password) => {
      try {
        await axiosInstance.delete(`${API}/file/${selectedFolder}/${filename}`, {
          data: { password },
        });
  ...
  const handleDownload = async (filename) => {
    ...
    window.open(`${configJson.apiBaseUrl}${API}/download/${selectedFolder}/${filename}?password=${encodeURIComponent(accessPassword)}`, '_blank');
  };
  ...
  const handlePasswordAction = async () => {
    try {
      if (passwordAction === 'set') {
        ...
        await axiosInstance.delete(`${API}/folder/password/${selectedFolderForPassword}`, {
      ...
      showToast(err.response?.data?.message || '❌ Lỗi xử lý mật khẩu', 'error');
    }
  };
  ...
  return (
    <Box p={3} sx={{ display: 'flex', height: '90vh', gap: 2 }}>
      {/* Sidebar */}
      <Paper sx={{
        width: 300, p: 2, overflowY: 'auto', height: '100%', 
        ...
        <Box sx={{ position: 'sticky', top: 0, background: theme.palette.background.component, zIndex: 2,display:'flex', justifyContent: 'space-between' }}>
          <Typography variant="h6">📁 Folders</Typography>
          <Button size='small' onClick={() => setNewFolderDialogOpen(true)}><AddOutlined></AddOutlined></Button>
        </Box>
        ...
        <List dense>
          {folders.map((folder) => (
            ...
            >
              <ListItemText primary={folder.name} secondary={folder.hasPassword ? '🔒 Bảo mật' : ''} />
              <ListItemSecondaryAction>
                {folder.hasPassword ? (
                  <IconButton onClick={() => openPasswordDialog(folder.name, 'update')}>
                    <SyncLockOutlined />
                  </IconButton>
                ) : (
                  <IconButton onClick={() => openPasswordDialog(folder.name, 'set')}>
                    <LockOpenIcon  color='primary' />
                  </IconButton>
                )}
                 {folder.hasPassword ?
                  <IconButton onClick={() => openPasswordDialog(folder.name, 'remove')}>
                    <LockOutlined color="warning" />
                  </IconButton>
                :''}
                <IconButton edge="end" onClick={() => handleDeleteFolder(folder.name)}>
      ...
      <Box flex={1} sx={{
        ...
        <Box sx={{ position: 'sticky', height: '150px', padding: '0px 10px', top: 0, background: theme.palette.primary.background, zIndex: 2 }}>
          <Typography variant="h6" gutterBottom>
            📄 Files in: <strong> / {selectedFolder || 'Chưa chọn folder'}</strong>
          </Typography>
          {selectedFolder && (
            <Stack direction="row" spacing={2} alignItems="center" sx={{ mt: 1, height: '50px' }}>
              <Button
        ...
        <Box sx={{
          width: '100%', overflowY: 'auto', height: 'calc(100% - 150px)',
          position: 'relative',
          '&::-webkit-scrollbar': { width: 6, opacity: 0 },
          '&:hover::-webkit-scrollbar': { width: 6, opacity: 1 },
          '&::-webkit-scrollbar-thumb': { backgroundColor: '#cdcdcd8c', borderRadius: '10px' }
          ...
          <List dense>
            {sortedFiles.map((file) => (
              <ListItem key={file.name} divider>
                <ListItemText primary={file.name} secondary={`${formatBytes(file.size)} | ${formatDate(file.createAt)}`} />
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        <DialogTitle>
          Xác nhận xóa {deleteTarget.type === 'folder' ? 'folder' : 'file'} "{deleteTarget.name}"
        </DialogTitle>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleConfirmDelete}>Xác nhận</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={passwordDialogOpen} onClose={() => setPasswordDialogOpen(false)}>
        <DialogTitle>
          {passwordAction === 'set' ? 'Cài đặt mật khẩu' : 
           passwordAction === 'update' ? 'Cập nhật mật khẩu' : 'Xóa mật khẩu'}
        </DialogTitle>
        <DialogContent sx={{ paddingTop: '8px !important' }}>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handlePasswordAction}>Xác nhận</Button>
        </DialogActions>
      </Dialog>

      {/* Folder access dialog */}
      <Dialog open={accessDialogOpen} onClose={() => setAccessDialogOpen(false)}>
        <DialogTitle>Nhập mật khẩu cho folder "{selectedFolderForPassword}"</DialogTitle>
        <DialogContent sx={{ paddingTop: '8px !important' }}>
          <TextField
            label="Mật khẩu"
            type="password"
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleAccessFolder}>Xác nhận</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={newFolderDialogOpen} onClose={() => setNewFolderDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button fullWidth variant="contained" onClick={createFolder}>Tạo folder</Button>
        </DialogActions>
      </Dialog>
