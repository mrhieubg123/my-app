...
const IssueChart = ({ idata = [], onSelectIssue, issueSelected }) => {
  ...
  idata.forEach((item) => {
    if (tempError2[item.ERROR_DESC]) {
      tempError2[item.ERROR_DESC] += item.QTY;
    } else {
      tempError2[item.ERROR_DESC] = item.QTY;
    }
    totalError += item.QTY;
  ...
  const columnData = categories.map((cat, i) => ({
    y: dataProduct[i] ?? 0,
    color: cat === issueSelected ? columnSelectedColor : columnBaseColor,
  }));

  useEffect(() => {
    if (categories.length > 0 && issueSelected === "" && idata.length > 0) {
      onSelectIssue(categories[0]);
    }
  }, [categories, issueSelected, idata]);
