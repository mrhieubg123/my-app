...
const IssueBySlotChart = ({
  ...
  idata
    .filter(
      (e) => e.ERROR_DESC === issueSelected && e.MODEL_NAME === modelSelected
    )
    .forEach((item) => {
      if (tempError2[item.LOCATION_CODE]) {
        tempError2[item.LOCATION_CODE] += item.QTY;
      } else {
        tempError2[item.LOCATION_CODE] = item.QTY;
      }
      totalError += item.QTY;
  ...
  const columnData = categories.map((cat, i) => ({
    y: dataProduct[i] ?? 0,
    color: cat === slotSelected ? columnSelectedColor : columnBaseColor,
  }));

  useEffect(() => {
    if (categories.length > 0 && slotSelected === "") {
      onSelectSlot(categories[0]);
    }
  }, [categories, slotSelected]);
...
export default IssueBySlotChart;
