...
const CABChart = ({idata =[], queryJSON = {}, onChangeDate}) => {
    ...
    const temp = {};
    idata && idata.forEach(item => {
            if (temp[item.DATET]) {
                temp[item.DATET].VOK += item.OK * 1;
                temp[item.DATET].VNG += item.NG * 1;
            }
            else {
                temp[item.DATET] = { DATET: item.DATET, VOK: item.OK * 1, VNG: item.NG * 1 };
            }
        });
  ...
  const options = {
    ...
    series: [{
      ...
      dataLabels:{
        ...
        type: 'spline',
        yAxis:0,
        data:  ErrorList.map(item => parseFloat((item["VNG"] * 100 / (item["VOK"] + item["VNG"]) * 1).toFixed(2))),
        color: "#ff3110",
        dataLabels:{
          color: '#ff3110',
          fontSize: "11px",
...
export default React.memo(CABChart);
